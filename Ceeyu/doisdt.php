...
<script>
  ...
  function myfuncmess(){
    ...
    <p style="text-align: center;margin-top:100px;font-size: 22px;font-family: 'Times New Roman', Times, serif;"><a id='backkk'onclick="history.back()"><i class='fa-solid fa-chevron-left'></i></a>Hồ sơ của tôi &nbsp > &nbsp Đổi Số Điện Thoại</p>
 ...
 <br>
 <div id="divpas2">
<form method="POST" onsubmit="return phone()" name="f1"  acction="doisdt.php" class='classpas'>
<?php
   include 'connection.php';
   $sql="SELECT*FROM member where id=".$_SESSION['id']." ";
   $result=$conn->query($sql);
   while($row=$result->fetch_assoc()){
     echo "   <input type='hidden' name='nghianhan'  value='".$row['phone']."'>
     <input type='hidden' name='iduser'  value='".$row['id']."'>
     ";
   
   
   
     echo "<p style='text-align:center'>Số điện thoại&nbsp&nbsp<span>";
        $forpas=$row['phone'];
    } ?> <?php
        $dem=strlen($forpas)-2;
  ...
  <input  type='text' placeholder='Nhập số điện thoại hiện tại'name="phonehientai"id="byphone"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"    maxlength='11'required><br>
   <div style="color:red" id="checkmk"> </div>
   <input  type='hidden' placeholder='Nhập số điện thoại mới' id="anphone" name="phonenew"oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" maxlength='11'><br>
   <div style="color:red" id="checkcc"></div>

   <input type='submit' class="buttondoimkuser" name="thaydoiphone" value='THAY ĐỔI' /> 
...
<nav style="padding-top:100px ;display: inline-block;position:fixed;top:570px ;left: 0px;">
   ...
   <script>
  function phone(){  
let phone=document.f1.nghianhan.value;  
let phonehientai=document.f1.phonehientai.value;  
let phonenew=document.f1.phonenew.value; 
if(!phonenew){
if(phonehientai==phone){  
  document.getElementById("byphone").style.borderColor="gray";
  document.getElementById("checkmk").innerHTML = "";
    document.getElementById("anphone").type="text";
return false;
}  
else{  
  document.getElementById("byphone").style.borderColor="red";
  document.getElementById("anphone").type="hidden";
    document.getElementById("checkmk").innerHTML = "Số điện thoại không đúng.";
