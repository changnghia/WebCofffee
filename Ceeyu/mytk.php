...
<script>
  ...
  function myfuncmess(){
    ...
    <p style="text-align: center;margin-top:100px;font-size: 22px;font-family: 'Times New Roman', Times, serif;">Hồ sơ của tôi</p>
 ...
 <div id='profile'>
  ...
  <?php 
  if(isset($_SESSION['ten'])&&isset($_SESSION['id'])){
    $id=$_SESSION['id'];
    ...
    while($row=$result->fetch_assoc()){
        ...
        <div id='profileleft'>
        <div id='divname'>
        <p id='forname'>Tên đăng nhập <span>".$row['username']."</span></p>
        </div>
        <p>Tên <input name='ten' type='text' size='40' value='".$row['ten']."'></p>
        <div id='divemail'>
        <p id='forname'>Email &nbsp&nbsp  <span>";?><?php 
        $foremail=$row['email'];


$vtemail=strstr($foremail,"@",true);
$dememail=strlen($vtemail)-2;
$hienthi=substr($foremail,0,2);
echo $hienthi;
$i=1;
while($i<=$dememail){
$thayemail="*";
...
$resultemail = str_replace($vtemail,"",$foremail);
echo $resultemail;
echo "<br>";
         echo"</span>&nbsp &nbsp <a href='doiemail.php'>Thay đổi</a></p>
        </div>
        
        <div id='divname'>
        <p id='forname'>Số điện thoại &nbsp&nbsp&nbsp<span>";?><?php
        $forpas=$row['phone'];
        $dem=strlen($forpas)-2;
        $vt=substr($forpas,$dem);
        $i=1;
        while($i<=$dem){
        $thay="*";
        echo "$thay";
        ...
        echo $vt;
        echo "
        </span> &nbsp&nbsp<a href='doisdt.php'>Thay đổi</a></p>
        </div>
        <p id='pgender'>Giới tính</p>
        <input";?> <?php 
        $sql_gd="SELECT*FROM member where id=".$_SESSION['id']."";
        $result_gd=$conn->query($sql);
        while($row_gd=$result_gd->fetch_assoc()){
          if($row_gd['gender']=='male'){
        echo"checked";
      }
      } ?> <?php echo" type='radio' id='forgender' value='male' name='gender'>
        <label for='forgender' id='labelgender'>Nam</label>

        <input ";?> <?php 
        $sql_gd="SELECT*FROM member where id=".$_SESSION['id']."";
        $result_gd=$conn->query($sql);
        while($row_gd=$result_gd->fetch_assoc()){
          if($row_gd['gender']=='female'){
        echo"checked";
      }
      } ?> <?php echo" type='radio' id='forgendernu' value='female' name='gender'>
        <label for='forgendernu' id='labelgendernu'>Nữ</label>

        <input";?> <?php 
        $sql_gd="SELECT*FROM member where id=".$_SESSION['id']."";
        $result_gd=$conn->query($sql);
        while($row_gd=$result_gd->fetch_assoc()){
          if($row_gd['gender']=='khac'){
        echo"checked";
      }
      } ?> <?php echo"  type='radio' id='forgenderkhac' value='khac' name='gender'>
        <label for='forgenderkhac' id='labelgenderkhac'>Khác</label>
        <p id='fordate'>Ngày sinh <input type='date'name='data' size='40'  value='".$row['data']."'></p>
        <p>Địa chỉ <input type='text' size='40' name='address' value='".$row['address']."'></p>
        <br>
      <button type='submit' name='suaprofile' id='forsmp'>Lưu</button>
  
        </div>
<div id='divavt'>
<div id='changeavt'><img  id='changnghia' src='http://localhost/CEEYU/image/".$row['avt']."' width='140px'height='140px' style='border-radius:500px'></div>
<label for='foravt' id='foravtlb'>Chọn Ảnh
<input onchange='changeFile(this)' name='avt' type='file' id='foravt'>
</label>
</div>
        </form>
        ";
  ...
  ?>
  <div id='divpas'><a href='doimkuser.php'><i class="fa-solid fa-key"> </i> Đổi mật khẩu</a></div>
</div>
    </div>
