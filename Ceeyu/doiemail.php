...
<script>
  ...
  function myfuncmess(){
    ...
    <p style="text-align: center;margin-top:100px;font-size: 22px;font-family: 'Times New Roman', Times, serif;"><a id='backkk'onclick="history.back()"><i class='fa-solid fa-chevron-left'></i></a>Hồ sơ của tôi &nbsp > &nbsp Đổi Email</p>
 ...
 <br>
 <div id="divpas2">
<form method="POST" onsubmit="return email()" name="f1"  acction="doiemail.php" class='classpas'>
<?php
   include 'connection.php';
   $sql="SELECT*FROM member where id=".$_SESSION['id']." ";
   $result=$conn->query($sql);
   while($row=$result->fetch_assoc()){
     echo "   <input type='hidden' name='emailht'  value='".$row['email']."'>
     <input type='hidden' name='iduser'  value='".$row['id']."'>
     ";
   
   
   
     echo "   <p style='text-align:center'>Email &nbsp&nbsp  <span>";
     $foremail=$row['email'];}
     ?><?php 
...
echo "<br>";
  ...
  <input  type='email' placeholder='Nhập email hiện tại'name="emailss"id="byphone"    required><br>
   <div style="color:red" id="checkmk"> </div>
   <input  type='hidden' placeholder='Nhập email mới' id="anphone" name="emailnew"><br>
   <div style="color:red" id="checkcc"></div>

   <input type='submit' class="buttondoimkuser" name="thaydoiemail" value='THAY ĐỔI' /> 
...
<nav style="padding-top:100px ;display: inline-block;position:fixed;top:570px ;left: 0px;">
   ...
   <script>
  function email(){  
let phone=document.f1.emailht.value;  
let phonehientai=document.f1.emailss.value;  
let phonenew=document.f1.emailnew.value; 
if(!phonenew){
if(phonehientai==phone){  
  document.getElementById("byphone").style.borderColor="gray";
  document.getElementById("checkmk").innerHTML = "";
    document.getElementById("anphone").type="email";
return false;
}  
else{  
  document.getElementById("byphone").style.borderColor="red";
  document.getElementById("anphone").type="hidden";
    document.getElementById("checkmk").innerHTML = "Email không đúng.";
