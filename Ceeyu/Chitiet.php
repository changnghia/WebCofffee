...
<head>
    ...
    <title><?php     
     ...
     if(isset($_GET['action']) && $_GET['action']=="hienthi"){ 
     $id=intval($_GET['id']); 
...
<script>
  ...
  function myfuncmess(){
    ...
    <p style="text-align: center;margin-top:100px;font-size: 15px;font-family: 'Times New Roman', Times, serif;"><a id='backkk'onclick="history.back()"><i class='fa-solid fa-chevron-left'></i></a>THỰC ĐƠN > <?php       $id=intval($_GET['id']);          
$sql="SELECT * FROM sanpham
    WHERE id={$id} " ;
    $result = $conn->query($sql);
      ...
      $id=intval($_GET['id']); 
          ...
          $sql="SELECT * FROM sanpham
              ...
              if ($result->num_rows > 0) {
                  ...
                  { 
         echo "
         <p id='hienthiten'>".$row['tensp']."</p>
         <img id='hienthianh' src='image/".$row['image']."'>
       <p id='hienthima'> Mã sản phẩm: #".$row['id']." </p>
       <p id='hienthimota'> Mô tả: ".$row['mota']."</p>
        <p id='hienthigia'> Giá : <b>".$row['gia']."₫</b></p>
        <p id='hienthigoi'>Gọi đặt mua:<b>0911.4725.18</b></p>
        <form  method='GET' action='xulycard.php'>
        <input type='hidden' name='action' value='add'/>
        <input type='hidden' name='id' value='". $row['id']."'/>
        <input type='hidden' name='return_url' value='".$current_url."' />
        <input id='buttonadd1' type='submit' value='THÊM VÀO GIỎ HÀNG'/>
      </form>
